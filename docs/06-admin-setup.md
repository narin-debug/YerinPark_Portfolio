...
가로 스크롤 갤러리·프로젝트 그리드를 로그인해서 직접 추가/수정/삭제할 수 있는
관리자 기능(`/admin`)을 붙였다. 이 기능은 Supabase(DB + 이미지 저장소)를
사용하므로, 배포 전에 아래 설정을 한 번 해줘야 한다.

## 1. Supabase 프로젝트 만들기

1. https://supabase.com 에서 무료 계정 생성 후 새 프로젝트(Project) 생성
2. 프로젝트 대시보드 → **SQL Editor** → New query에서 이 저장소의
   `supabase/schema.sql` 내용을 그대로 붙여넣고 실행 (테이블 2개 생성:
   `gallery_items`, `projects`)
...
`/admin/gallery`, `/admin/projects`에서 항목을 추가/수정/삭제할 수 있다.
메인 페이지 하단 좌측(저작권 표시 옆의 작은 점 `·`)에 숨겨진 관리자
링크가 있다.
...
- 갤러리/프로젝트에 등록된 항목이 하나도 없으면 기존 placeholder(회색
  ...
  순간부터 placeholder 대신 실제 등록한 항목들이 보인다.
